	...
	<div class="col">
		<section class="card">
			<header class="card-header">
				...
				<h2 class="card-title"><strong>Invoice Update By Admin</strong></h2>
			</header>
			<div class="card-body">
				
				@if($invoice)

				<form action="{{route('invoiceUpdate')}}" method="post" enctype="multipart/form-data">
                    @csrf
                        <?php
                        ...
                        form_hidden("id", $invoice->id);
                        form_textarea("sender_address", $invoice->sender_address);
                        form_textarea("receiver_address", $invoice->receiver_address);
                        form_input("order_no", $invoice->order_no);
                        form_input("delivery_through", $invoice->delivery_through);
                        form_input("carrier_name", $invoice->carrier_name);
                        form_input_number("remaining_order_amount", $invoice->remaining_order_amount);
                        form_input_number("shipping_cost_warehouse", $invoice->shipping_cost_warehouse);
                        form_input_number("delivery_cost_to_your_address", $invoice->delivery_cost_to_your_address);


                        $other_fees_name = json_decode($invoice->other_fees_name);
                        $other_fees_value = json_decode($invoice->other_fees_value);
                        ...
                        form_input_number("insurence_fee", $invoice->insurence_fee);
                        form_input_number("custom_duities", $invoice->custom_duities);
                        form_input_number("vat", $invoice->vat);
                        form_select("discount_type", $amount_type, $invoice->discount_type, false);
                        form_input_number("discount_amount", $invoice->discount_amount, false);

                        form_input("total", $invoice->total);
